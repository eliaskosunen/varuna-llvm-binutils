...
GetOutputStream(const char* TargetName, Triple::OSType OS, const char* ProgName)
{
...
static int compileModule(char** argv, LLVMContext& Context)
{
    ...
    if(!SkipModule)
    {
        ...
        // Verify module immediately to catch problems before doInitialization()
        // is
...

    {
        ...
        if(!RunPassNames->empty())
        {
            ...
            LLVMTargetMachine& LLVMTM =
                static_cast<LLVMTargetMachine&>(*Target);
        ...
        // opt has the same functionality, so it may be worth abstracting this
        // out
        // in the future.
        ...
        if(CompileTwice)
        {
                       ...
                       Buffer.size()) != 0))
            {
                errs() << "Running the pass manager twice changed the output.\n"
                          "Writing the result of the second run to the "
                          "specified output\n"
